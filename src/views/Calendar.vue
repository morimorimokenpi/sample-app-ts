      ...
      >
        <v-card v-if="selectedEvent" color="grey lighten-4" flat>
          <v-toolbar :color="getEventColor(selectedEvent)" dark>
            ...
            <v-btn
              icon
              :disabled="isNotOwner(selectedEvent.useId)"
...
export default defineComponent({
  ...
  setup(props, context) {
    const state = reactive({
      ...
      // イベント開始日付を選択するv-date-pickerを表示するかどうかを示す値
      isOpenStartDatePicker: false,
      // v-date-pickerコンポーネントで選択したイベント開始日付
      startDate: null as string | null,
      // イベント開始時刻を選択するv-time-pickerコンポーネントを表示するかどうかを示す値
      isOpenStartTimePicker: false,
      // v-time-pickerコンポーネントで選択したイベント開始時刻
      startTime: null as string | null,
      // イベント終了日付を選択するv-date-pickerを表示するかどうかを示す値
      isOpenEndDatePicker: false,
      // v-date-pickerコンポーネントで選択したイベント終了日付
      endDate: null as string | null,
      // イベント終了時刻を選択するv-time-pickerコンポーネントを表示するかどうかを示す値
      isOpenEndTimePicker: false,
      // v-time-pickerコンポーネントで選択したイベント終了時刻
      ...
      title: computed((): string => {
        ...
        if (props.type === 'week') {
          return state.start.month === state.end.month
            ...
            : `${state.start.year}年 ${state.start.month}月〜${state.end.month}`;
        }
    ...
    const methods = {
      ...
      showEvent: ({
        ...
        // 実装はhttps://vuetifyjs.com/ja/components/calendars/#イベント参照。
      ...
      // イベントの日付選択ダイアログに表示する日のフォーマットを行う
      ...
      clearStartTime: () => {
        state.startTime = null;
        // イベント終了時刻のみの入力は出来ないため、イベント終了時刻もクリアする
        state.endTime = null;
      },
      // イベント開始時刻選択ダイアログを閉じる
